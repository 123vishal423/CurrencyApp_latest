...
class CurrencyDetailsViewModel {
                ...
                case .success(let dta) :
                    if let rates = dta[StringConstants.ratesKey] as? [String: Double] {
                        if let fromValue = rates[fromSymbol], let toValue = rates[toSymbol], let value = Double(valueToConvert) {
                            ...
                            let usd = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.dollarSymbol]!, valueToConvert: value)
                            let euro = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.euroSymbol]!, valueToConvert: value)
                            let gbp = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.britishPoundSymbol]!, valueToConvert: value)
                            let egp = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.egyptPoundSymbol]!, valueToConvert: value)
                            let cad = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.canadianDollarSymbol]!, valueToConvert: value)
                            let inr = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.indianRupeeSymbol]!, valueToConvert: value)
                            let aus = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.australianDollarSymbol]!, valueToConvert: value)
                            let jpy = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.japaneseYenSymbol]!, valueToConvert: value)
                            let cny = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.chineseSymbol]!, valueToConvert: value)
                            let qar = self.convertCurrency(fromValue: fromValue, toValue: rates[StringConstants.qatarRiyalSymbol]!, valueToConvert: value)

                            
                            historicalModel.append(HistoricalDataModel(fromCurrencySymbol: fromSymbol, fromCurrencyValue: valueToConvert, toCurrencySymobl: toSymbol, toCurrencyValue: convertedValue, dateString: dateString, dollarSymbol: usd, euroSymbol: euro, britishPoundSymbol: gbp, egyptPoundSymbol: egp, canadianDollarSymbol: cad, indianRupeeSymbol: inr, australianDollarSymbol: aus, japaneseYenSymbol: jpy, chineseSymbol: cny, qatarRiyalSymbol: qar))
                        }
                    }
                }
                dispatchGroup.leave()
    ...
    func getHistoricalDates() -> [String] {
        ...
        for  i in 0...2{
            var dateStr = ""
            if i == 0{
                dateStr = dateFormatter.string(from: NSDate() as Date)
                datesData.append(dateStr)
            }else{
                let lastWeekDate = NSCalendar.current.date(byAdding: .day, value: -(i), to: NSDate() as Date)
                dateStr = dateFormatter.string(from: lastWeekDate!)
                datesData.append(dateStr)
            }
